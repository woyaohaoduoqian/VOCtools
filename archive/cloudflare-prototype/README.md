# Cloudflare 原型（冻结，不部署）

本目录保留历史原型，当前不属于产品使用或发布路径。请勿运行 `npm run dev`、`npm run deploy`，也不要在此目录配置密钥；当前维护目标是根目录的 Codex 插件。若未来决定恢复独立 Web 服务，再基于当时的安全、鉴权和成本要求重新评估本目录。

# 历史说明：Cloudflare 发布版 VOC Research Agent
