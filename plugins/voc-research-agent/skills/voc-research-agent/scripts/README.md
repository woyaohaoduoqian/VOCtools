...
`voc_workbench.mjs` 与 `voc_workbench.py` 将现有四阶段规则落成可复算的本地交付物；它们不访问网络，不会调用 Apify，也不会读取或保存 token。两种入口等价：有 Python 环境时可使用 `.py`，有 Node.js 18+ 时可使用 `.mjs`。

## 典型顺序

1. `init`：建立任务目录与待确认方案模板。
2. 用户确认方案、授权采集后，按 `data-collection/SKILL.md` 采集并写入 `processed/`、`raw/` 与 `manifest.json`。
...
5. 智能体依据 `insight-writing/SKILL.md` 撰写 `report.md`；运行 `validate --for-report` 后使用 `pack` 打包交付。
