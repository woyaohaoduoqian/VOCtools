...
- **每个阶段落文件**:`plan.md` → `manifest.json` → `quality.md` → `report.md`。中断能续、下游能引、结论能溯源。
...
**待接入**
- Reddit —— 托管 MCP 因平台不支持 OAuth 无法绑定。备选:Apify 的 Reddit actor(付费,零新增基础设施)或 Reddit 官方 script 应用(免费,搜索上限 250 条)
- TikTok / Etsy —— 平台画像已写,能力规格未验证
- 国内平台 —— 合规问题未定,不接

**待核对**(以方法论原文为准,现为暂定值)
- `insight-writing`:完整证据强弱表、禁用词清单、编码一致率阈值(暂定不一致率 >20% 判编码不可靠)

**待确认**
- WinSight 是否支持 agent 级密钥配置(决定 Apify 凭证走哪条路)
- 文件空间是否跨会话持久(决定数据复用能不能跨调研)
