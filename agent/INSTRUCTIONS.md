...
8. **发现、解读与假设**：生成 `findings.csv`、`insights.md`、`hypotheses.csv`、`report.md`。
...
- 证据关系只认 `evidence-links.csv`。
- 样本内发现只认 `findings.csv`；需求解释只认 `insights.md`；产品假设只认 `hypotheses.csv`。

任何洞察必须沿 ID 链回到公开原文。不得在多个文件重复维护主类别、计数分母或运行状态。
