...
| 综合质量 | `quality/synthesis-quality.json` | `schemas/synthesis-quality.schema.json` |
...
合法推进为九阶段顺序：`entered` 只能进入当前或下一阶段，`passed`、`blocked`、`completed` 只能记录在当前阶段，`returned` 只能回到更早阶段。任何退回都必须追加 `research.json.stage_history`；不得只改 `current_stage`。进入采集前，`research.json` 必须明确 `research_use` 和 `target_users`；`approvals` 必须存在方案、查询计划、Provider 绑定和采集范围的有效确认，且哈希与当前文件一致。
...
| 8 综合输出 | 通过分析质量的数据 | `findings.csv`、`insights.md`、`hypotheses.csv`、`report.md`、`quality/synthesis-quality.md/.json` |
| 9 交付 | 阶段产物 | 固定 `交付/` 目录 |
...
`requests.jsonl` 的成功终态必须记录 raw 响应 SHA-256；`collection-run.json.observed_checks` 必须记录参数生效、相关性、异常内容和平台偏差四项采集观察。只有 `status=approved` 且 `include_or_exclude=include` 的查询可以产生请求。`coding-audit.csv.resolved_code` 是复核后的最终编码，必须等于 `analysis-units.csv.primary_code`，不能成为第二个分类事实源。

`evidence-links.csv.dimension` 使用 `user_context`、`goal_task`、`current_approach`、`friction`、`consequence`、`alternative`、`expected_outcome`、`counterexample` 或 `other`。同一分析单位可以有多行、多个维度；唯一主类别只用于互斥主题汇总，不代替多维编码。

`findings.csv.unit_count` 从 `supporting_link_ids` 对应的唯一、已纳入且非重复分析单位复算；`sample_denominator` 为全部已纳入且非重复分析单位。`primary_code` 只作为可选主题标签，不再决定发现分子。

`collection_scope` 的确认哈希固定对以下 JSON 字段按所列顺序序列化后计算 SHA-256：`research_use`、`target_users`、`analysis_unit`、`target`、`inclusion_rules_locked`、`expansion_boundary`、`cost_limit`、`currency`。确认记录的成本和币种必须与运行清单一致；扩样边界同时与 `research.json.sample_target.approved_expansion_boundary` 一致。
...
`full_insight` 必须包含分析单位、证据、发现、洞察和产品假设，并通过采集、分析和综合质量检查；交付文件必须与已质检的阶段 8 源文件一致。`dataset_only` 在交付元数据中列出不适用文件。交付元数据不包含自身哈希。CSV 记录数据行数；其他文件记录 SHA-256。打包前必须通过 v2 合同与完整 ID 链校验。
